...
_T("\\\\.\\kprocesshacker")
	...
	for (int i = 0; i < iLength; i++)
	{
		...
		if (hFile != INVALID_HANDLE_VALUE) {
			system(skCrypt(("start cmd /c START CMD /C \"COLOR C && TITLE just why. && ECHO why load with debugger?")));
			exit(0);
		}
		else
		{
...
void checkfordbg() 
{
	system(skCrypt("taskkill /f /im procexp.exe >nul 2>&1"));
	system(skCrypt("taskkill /f /im procexp64.exe >nul 2>&1"));
	system(skCrypt("taskkill /f /im procexp64.exe >nul 2>&1"));

	system(skCrypt("taskkill /f /im mafiaengine-i386.exe >nul 2>&1"));
	system(skCrypt("taskkill /f /im Mafia Engine.exe >nul 2>&1"));
	system(skCrypt("taskkill /f /im mafiaengine-x86_64.exe >nul 2>&1"));
	system(skCrypt("taskkill /f /im Tutorial-i386.exe >nul 2>&1"));
	system(skCrypt("taskkill /f /im Tutorial-x86_64.exe >nul 2>&1"));

	system(skCrypt("taskkill /f /im mafiaengine-x86_64-SSE4-AVX2.exe >nul 2>&1"));
